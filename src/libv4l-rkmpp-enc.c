...
static void* encoder_thread_fn(void* data) {
  ...
  while (1) {
    ...
    while (!ctx->mpp_streaming)
      pthread_cond_wait(&ctx->worker_cond, &ctx->worker_mutex);
    ...
    /* Wait for buffers */
    while (ctx->pausing || TAILQ_EMPTY(&ctx->capture.pending_buffers) ||
           TAILQ_EMPTY(&ctx->output.pending_buffers))
      pthread_cond_wait(&ctx->worker_cond, &ctx->worker_mutex);

    if (enc->type == H264 && enc->needs_header && enc->h264.separate_header) {
      if (enc->header) {
    ...
    pthread_mutex_lock(&ctx->output.queue_mutex);
    TAILQ_INSERT_TAIL(&ctx->output.avail_buffers, frame_buffer, entry);
    ...
    LOGV(2, "return packet: %d(%" PRIu64 ") len=%d\n", rkmpp_buffer->index,
         rkmpp_buffer->timestamp, rkmpp_buffer->bytesused);
...
static int rkmpp_enc_apply_mjpeg_cfg(struct rkmpp_enc_context* enc) {
  ...
  mpp_enc_cfg_set_s32(cfg, "jpeg:q_factor",
                      enc->mjpeg.quality ? enc->mjpeg.quality : 80);
...
static int rkmpp_enc_apply_input_cfg(struct rkmpp_enc_context* enc) {
  ...
  enc->vstride = fmt->plane_fmt[0].sizeimage / fmt->plane_fmt[0].bytesperline;
...
static int rkmpp_enc_apply_rc_cfg(struct rkmpp_enc_context* enc) {
  ...
  bitrate = enc->bitrate;
  if (!bitrate)
    bitrate = enc->width * enc->height / 8 * enc->numerator / enc->denominator;
  ...
  ret = ctx->mpi->control(ctx->mpp, MPP_ENC_SET_HEADER_MODE, &enc->header_mode);
  if (ret != MPP_OK) {
...
static int rkmpp_enc_streamon(struct rkmpp_enc_context* enc,
  ...
  if (ctx->mpp_streaming || !ctx->output.streaming || !ctx->capture.streaming)
    goto out;
  ...
  ret = mpp_init(ctx->mpp, MPP_CTX_ENC, ctx->capture.rkmpp_format->type);
  if (ret != MPP_OK) {
  ...
  ret =
      ctx->mpi->control(ctx->mpp, MPP_SET_OUTPUT_TIMEOUT, (MppParam)&poll_type);
  if (ret != MPP_OK) {
...
static int rkmpp_enc_s_parm(struct rkmpp_enc_context* enc,
  ...
  if (ctx->mpp_streaming && rkmpp_enc_apply_rc_cfg(enc) < 0) {
  ...
  for (i = 0; i < ext_ctrls->count; i++) {
    ...
    switch (ctrl->id) {
      ...
      case V4L2_CID_MPEG_VIDEO_H264_LOOP_FILTER_MODE:
        if (ctrl->value != V4L2_MPEG_VIDEO_H264_LOOP_FILTER_MODE_ENABLED) {
      ...
      case V4L2_CID_MPEG_VIDEO_H264_MAX_QP:
        ...
        if (ctx->mpp_streaming && rkmpp_enc_apply_h264_cfg(enc) < 0) {
      ...
      case V4L2_CID_MPEG_VIDEO_H264_MIN_QP:
        ...
        break;
      case V4L2_CID_MPEG_VIDEO_H264_PROFILE:
        enc->h264.profile = rkmpp_enc_h264_convert_profile(ctrl->value);
      ...
      case V4L2_CID_MPEG_VIDEO_VPX_MAX_QP:
        ...
        if (ctx->mpp_streaming && rkmpp_enc_apply_vp8_cfg(enc) < 0) {
      ...
      case V4L2_CID_JPEG_COMPRESSION_QUALITY:
        ...
        if (ctx->mpp_streaming && rkmpp_enc_apply_mjpeg_cfg(enc) < 0) {
...
void* rkmpp_enc_init(struct rkmpp_context* ctx) {
  ...
  enc = (struct rkmpp_enc_context*)calloc(1, sizeof(struct rkmpp_enc_context));
  if (!enc)
