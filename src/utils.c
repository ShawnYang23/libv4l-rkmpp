...
static int rkmpp_copy_buffer(struct rkmpp_context* ctx,
  ...
  for (i = 0; i < buffer->length; i++) {
    ...
    if (!sizes[i]) {
      LOGV(3,
           "index:%d,planes[%u].size :%u, offset:%u, buffer->length:%u, "
           "bytesused:%u\n",
           rkmpp_buffer->index, i, sizes[i], offsets[i], buffer->length,
           rkmpp_buffer->planes[i].bytesused);
      goto out;
    }
    if (buffer->memory == V4L2_MEMORY_DMABUF) {
      int fd = rkmpp_buffer->planes[i].fd;
      addrs[i] = mmap(NULL, sizes[i], PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                      offsets[i]);
      err = errno;
      if (addrs[i] == MAP_FAILED) {
        LOGV(3,
             "mmap failed: i:%u, fd:%d, size:%u, offsets:%u, errno:%d, "
             "sizeimage:%u, planes[0].length:%u\n",
...
int rkmpp_to_v4l2_buffer(struct rkmpp_context* ctx,
  ...
  for (i = 0; i < buffer->length; i++) {
    ...
    buffer->m.planes[i].data_offset = rkmpp_buffer->planes[i].data_offset;
    buffer->m.planes[i].bytesused = 0;

    if (buffer->memory == V4L2_MEMORY_MMAP)
      /* Only support mem_offset for plane 0 */
      buffer->m.planes[i].m.mem_offset =
          i ? 0 : RKMPP_MEM_OFFSET(buffer->type, buffer->index);
    else if (buffer->memory == V4L2_MEMORY_USERPTR)
      buffer->m.planes[i].m.userptr = rkmpp_buffer->planes[i].userptr;
    else if (buffer->memory == V4L2_MEMORY_DMABUF)
      buffer->m.planes[i].m.fd = rkmpp_buffer->planes[i].fd;
  }
  if (output)
    LOGV(3, "deque output buffer, length:%d, fd(0):%d \n", buffer->length,
         rkmpp_buffer->planes[0].fd);
  else
  ...
  if (rkmpp_buffer_available(rkmpp_buffer) && rkmpp_buffer->bytesused) {
    ...
    if (!rkmpp_buffer->planes[0].plane_size)
      rkmpp_buffer->planes[0].plane_size =
          rkmpp_buffer->bytesused - rkmpp_buffer->planes[0].data_offset;
...
int rkmpp_from_v4l2_buffer(struct rkmpp_context* ctx,
  ...
  for (i = 0; i < buffer->length; i++) {
    ...
    rkmpp_buffer->planes[i].plane_size =
        rkmpp_buffer->planes[i].bytesused - rkmpp_buffer->planes[i].data_offset;
  ...
  rkmpp_buffer->timestamp =
      (uint64_t)buffer->timestamp.tv_sec * 1000000 + buffer->timestamp.tv_usec;
  ...
  if (!input)
    LOGV(2, "enque output buffer, length:%d, fd(0):%d, status:0x%x\n",
         ...
         fcntl(buffer->m.planes[0].m.fd, F_GETFD));
  else
...
void log_prefix(const char* prefix,
  ...
  snprintf(buf, sizeof(buf),
           "[%03ld.%03ld] [%s] [%s:%d] [err:%d]: ", tv.tv_sec % 1000,
           tv.tv_usec / 1000, prefix, func, line, code);
  va_list args;
...
void dump_rkmpp_format(const struct rkmpp_fmt* fmt, const char* prefix) {
  ...
  LOGV(2, "frmsize: %dx%d %dx%d %dx%d\n", fmt->frmsize.min_width,
       fmt->frmsize.max_width, fmt->frmsize.min_height, fmt->frmsize.max_height,
