...
static void rkmpp_put_packets(struct rkmpp_dec_context* dec) {
  ...
  while (!TAILQ_EMPTY(&ctx->output.pending_buffers)) {
    ...
    if (rkmpp_buffer == &ctx->eos_buffer) {
      ...
      mpp_packet_init(&packet, mpp_buffer_get_ptr(rkmpp_buffer->rkmpp_buf),
...
static void rkmpp_apply_info_change(struct rkmpp_dec_context* dec,
  ...
  if (!memcmp((void*)&video_info, (void*)&dec->video_info,
              sizeof(video_info))) {
    LOGV(1, "ignore unchanged frame info\n");

    ctx->mpi->control(ctx->mpp, MPP_DEC_SET_INFO_CHANGE_READY, NULL);
    return;
  }
...
static void* decoder_thread_fn(void* data) {
  ...
  while (1) {
    ...
    if (mpp_frame_get_errinfo(frame) || mpp_frame_get_discard(frame)) {
      ...
      /* Size of NV12 image */
      rkmpp_buffer->bytesused =
          dec->video_info.hor_stride * dec->video_info.ver_stride * 3 / 2;
    }
...
static int rkmpp_dec_g_fmt(struct rkmpp_dec_context* dec,
  ...
  if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE && !dec->video_info.valid) {
...
static int rkmpp_dec_streamon(struct rkmpp_dec_context* dec,
  ...
  ret = mpp_init(ctx->mpp, MPP_CTX_DEC, ctx->output.rkmpp_format->type);
  if (ret != MPP_OK) {
  ...
  split = 0;
  ret = ctx->mpi->control(ctx->mpp, MPP_DEC_SET_PARSER_SPLIT_MODE, &split);
  if (ret != MPP_OK) {
  ...
  ret = ctx->mpi->control(ctx->mpp, MPP_DEC_SET_EXT_BUF_GROUP,
...
void* rkmpp_dec_init(struct rkmpp_context* ctx) {
  ...
  pthread_create(&ctx->worker_thread, NULL, decoder_thread_fn, dec);
