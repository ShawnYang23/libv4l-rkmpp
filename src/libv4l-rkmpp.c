...
static const struct rkmpp_fmt* rkmpp_find_fmt(struct rkmpp_context* ctx,
                                              uint32_t fourcc) {
  unsigned int i;
...
int rkmpp_enum_fmt(struct rkmpp_context* ctx, struct v4l2_fmtdesc* f) {
  ...
  for (i = 0, j = 0; i < ctx->num_formats; ++i) {
    ...
    if (j == f->index) {
      strncpy((char*)f->description, fmt->name, sizeof(f->description) - 1);
...
static void calculate_plane_sizes(const struct rkmpp_fmt* fmt,
  ...
  for (i = 0; i < fmt->num_planes; ++i) {
    ...
    pix_fmt_mp->plane_fmt[i].sizeimage =
        h * pix_fmt_mp->plane_fmt[i].bytesperline;
    /*
...
int rkmpp_try_fmt(struct rkmpp_context* ctx, struct v4l2_format* f) {
  ...
  if (compressed) {
    ...
    *pix_fmt_mp = queue->format;
    ...
    pix_fmt_mp->width =
        clamp(pix_fmt_mp->width, queue->rkmpp_format->frmsize.min_width,
              queue->rkmpp_format->frmsize.max_width);
    pix_fmt_mp->height =
        clamp(pix_fmt_mp->height, queue->rkmpp_format->frmsize.min_height,
...
int rkmpp_s_fmt(struct rkmpp_context* ctx, struct v4l2_format* f) {
  ...
  queue->rkmpp_format = rkmpp_find_fmt(ctx, pix_fmt_mp->pixelformat);
...
int rkmpp_reqbufs(struct rkmpp_context* ctx,
  ...
  queue->buffers = (struct rkmpp_buffer*)calloc(queue->num_buffers,
                                                sizeof(struct rkmpp_buffer));
  if (!queue->buffers)
    goto err;

  /* Allocate all buffers from main buffer pool */
  for (i = 0; i < queue->num_buffers; i++) {
    ret = mpp_buffer_get(queue->internal_group, &buffer, sizeimage);
    if (ret != MPP_OK) {
...
int rkmpp_querybuf(struct rkmpp_context* ctx, struct v4l2_buffer* buffer) {
  ...
  ret =
      rkmpp_to_v4l2_buffer(ctx, rkmpp_buffer, buffer,
...
int rkmpp_expbuf(struct rkmpp_context* ctx, struct v4l2_exportbuffer* expbuf) {
  ...
  LOGV(1, "export buf(%d), type: %d, fd: %d(%d)\n", expbuf->index, expbuf->type,
       expbuf->fd, rkmpp_buffer->fd);
...
int rkmpp_qbuf(struct rkmpp_context* ctx, struct v4l2_buffer* buffer) {
  ...
  ret =
      rkmpp_from_v4l2_buffer(ctx, buffer, rkmpp_buffer,
  ...
  pthread_mutex_lock(&queue->queue_mutex);
  TAILQ_INSERT_TAIL(&queue->pending_buffers, rkmpp_buffer, entry);
  rkmpp_buffer_set_pending(rkmpp_buffer);
  pthread_mutex_unlock(&queue->queue_mutex);

  LOGV(2, "enqueue buffer: %d(%ld), size: %d, type: %d, fd: %d\n",
       buffer->index, buffer->timestamp.tv_sec, rkmpp_buffer->bytesused,
       buffer->type, rkmpp_buffer->fd);
...
int rkmpp_dqbuf(struct rkmpp_context* ctx, struct v4l2_buffer* buffer) {
  ...
  LOGV(2, "dequeue buffer: %d(%" PRIu64 "), size: %d, type: %d\n",
       buffer->index, rkmpp_buffer->timestamp, rkmpp_buffer->bytesused,
       buffer->type);
...
void rkmpp_start_flushing(struct rkmpp_context* ctx) {
  ...
  pthread_mutex_lock(&ctx->output.queue_mutex);
  TAILQ_INSERT_TAIL(&ctx->output.pending_buffers, &ctx->eos_buffer, entry);
...
static int plugin_ioctl(void* dev_ops_priv,
                        int fd,
                        unsigned long cmd,
                        void* arg) {
  (void)fd; /* unused */
...
static void* plugin_mmap(void* dev_ops_priv,
                         void* start,
                         size_t length,
                         int prot,
                         int flags,
