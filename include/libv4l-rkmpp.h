...
__attribute__((format(printf, 5, 6))) void log_prefix(const char* prefix,
                                                      const char* func,
                                                      int line,
                                                      int code,
                                                      const char* format,
...
#define CLEAR_AFTER_FIELD(p, field)                                          \
  memset((uint8_t*)(p) + offsetof(typeof(*(p)), field) + sizeof((p)->field), \
...
#define RKMPP_MEM_OFFSET(type, index) ((int64_t)((type) << 16 | (index)))
...
static inline struct rkmpp_buf_queue* rkmpp_get_queue(struct rkmpp_context* ctx,
...
static inline const char* rkmpp_cmd2str(unsigned long cmd) {
#define RKMPP_CMD2STR(cmd) \
  case cmd:                \
    return #cmd
